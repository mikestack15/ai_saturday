...
#the order dollar amount total of a given dining experience
...
train <- train[c("order_dollar_amount","table_party_size","overall_review","duration_of_stay",
                 "table_wait_time")]
test <- test[c("order_dollar_amount","table_party_size","overall_review","duration_of_stay",
               "table_wait_time")]
...
labels <- train$order_dollar_amount
ts_label <- test$order_dollar_amount

new_tr <- model.matrix(~.+0,data=train[,-c("order_dollar_amount"),with=F])
new_ts <- model.matrix(~.+0,data=test[,-c("order_dollar_amount"),with=F])
...
for(i in unique(params$index)) {
  ...
  actual <- test$order_dollar_amount
...
final_forecast_df <- data.frame(test_bkp,total_spent_predictions=preds)
final_forecast_df <- final_forecast_df[,c("Experience_id","total_spent_predictions")]
